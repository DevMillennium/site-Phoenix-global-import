    ...
    <header className="sticky top-0 z-[9999] isolate w-full border-b border-phoenix-border bg-phoenix-dark/95 backdrop-blur supports-[backdrop-filter]:bg-phoenix-dark/80 pointer-events-auto pt-[env(safe-area-inset-top)]">
      <div className="container flex h-14 min-h-touch sm:h-16 items-center justify-between gap-2 relative z-[1] pointer-events-auto">
        ...
        <form
          action="/produtos"
          method="get"
          className="relative z-[2] hidden min-[900px]:flex flex-1 max-w-md mx-4 pointer-events-auto"
          role="search"
        ...
        >
          ...
          <input
            ...
            placeholder="Buscar..."
            autoComplete="off"
            className="w-full rounded-lg border border-phoenix-border bg-phoenix-card py-2 pl-3 pr-10 text-sm text-phoenix-text placeholder:text-phoenix-muted focus-visible:outline focus-visible:outline-2 focus-visible:outline-phoenix-primary min-h-10"
          />
          <button
            type="submit"
            className="absolute right-1 top-1/2 -translate-y-1/2 flex h-8 w-8 items-center justify-center rounded-md text-phoenix-text-muted hover:text-phoenix-primary transition-colors"
            aria-label="Buscar"
          >
      ...
      <AnimatePresence>
        {mobileOpen && (
          ...
          >
            <nav className="container flex flex-col py-4 gap-3 px-4 sm:px-6" aria-label="Menu mobile">
              <form action="/produtos" method="get" className="flex gap-2" role="search" aria-label="Buscar produtos">
                <input
                  name="q"
                  type="search"
                  placeholder="Buscar produtos..."
                  className="flex-1 rounded-lg border border-phoenix-border bg-phoenix-card px-3 py-2.5 text-sm text-phoenix-text min-h-touch"
                  autoComplete="off"
                />
                <button
                  type="submit"
                  className="rounded-lg bg-phoenix-primary px-4 py-2.5 text-sm font-medium text-white min-h-touch"
                >
                  Buscar
                </button>
              </form>
