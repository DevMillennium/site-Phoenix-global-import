...
import { BotaoPagarCartaoCarrinho } from "@/components/checkout/BotaoPagarCartaoCarrinho";
import { getCrossSellForCart } from "@/lib/cart-cross-sell";

export function CartContent() {
  const { items, removeItem, updateQuantity, totalItems, totalAmount } = useCart();
  const slugSet = useMemo(() => new Set(items.map((i) => i.slug)), [items]);
  const crossSell = useMemo(() => getCrossSellForCart(slugSet, 4), [slugSet]);
      ...
      <>
        <div className="mt-8 sm:mt-12 rounded-xl border border-phoenix-border bg-phoenix-card p-6 sm:p-8 text-center">
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-3 sm:gap-4">
            ...
            <LinkWhatsApp
              ...
              trackingSource="cart_empty"
              trackingAction="falar_loja"
            >
              Falar no WhatsApp
            </LinkWhatsApp>
          </div>
        ...
        <div className="mt-10 rounded-xl border border-phoenix-border bg-phoenix-surface/50 p-6">
          ...
          <ol className="mt-3 list-inside list-decimal space-y-2 text-sm text-phoenix-text-muted">
            ...
            <li>No carrinho, finalize com &quot;Pagar carrinho com cartão&quot; (Stripe) ou envie a lista pelo WhatsApp.</li>
            <li>Receba confirmação e combine frete quando necessário.</li>
  ...
  return (
    <>
      <FreeShippingBar subtotalBrl={totalAmount} className="mt-8 sm:mt-10" />

      <div className="mt-6 sm:mt-8 rounded-xl border border-phoenix-border bg-phoenix-card overflow-hidden">
        ...
        <div className="border-t border-phoenix-border p-4 sm:p-6 space-y-4">
          ...
          <p className="text-sm text-phoenix-text-muted">
            Escolha como finalizar: pagamento com cartão (Stripe) ou envio da lista pelo WhatsApp para combinar PIX e frete.
          </p>
          <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-stretch">
            <BotaoPagarCartaoCarrinho className="flex-1 sm:flex-initial min-w-[min(100%,14rem)]" />
            <LinkWhatsApp
              href={whatsAppHref}
              className="inline-flex flex-1 sm:flex-initial min-w-[min(100%,14rem)] items-center justify-center min-h-touch gap-2 rounded-lg bg-phoenix-success px-6 py-3 font-medium text-white hover:opacity-90 transition-opacity focus-visible:outline focus-visible:outline-2 focus-visible:outline-phoenix-primary touch-manipulation"
              ariaLabel="Enviar lista do carrinho pelo WhatsApp"
              trackingSource="cart"
              trackingAction="enviar_lista"
            >
      ...
      <CrossSellRow products={crossSell} />

      <div className="mt-10 rounded-xl border border-phoenix-border bg-phoenix-surface/50 p-6">
        <h2 className="font-semibold text-phoenix-text">Como comprar</h2>
        <ol className="mt-3 list-inside list-decimal space-y-2 text-sm text-phoenix-text-muted">
          <li>Revise os itens acima e use &quot;Pagar carrinho com cartão&quot; ou &quot;Enviar lista no WhatsApp&quot;.</li>
          <li>No Stripe, o valor segue os preços do site; no WhatsApp você combina PIX, entrega e retirada.</li>
          <li>Estoque em Fortaleza — enviamos para todo o Brasil.</li>
        </ol>
      </div>
    </>
